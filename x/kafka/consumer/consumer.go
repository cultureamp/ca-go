...
// processed in order.
type GetBatchKey func(message kafka.Message) string
...
func NewConsumer(dialer *kafka.Dialer, config Config, opts ...Option) *Consumer {
	...
	c := &Consumer{
		...
		batchSize:  0,
		batchKeyFn: func(message kafka.Message) string { return "" },
	}
	...
	for {
		...
		if c.batchSize > 0 {
			if err := c.processBatch(ctx, handler); err != nil {
				return fmt.Errorf("consumer %s unable to process message batch: %w", c.id, err)
			}
		} else {
			if err := c.process(ctx, handler); err != nil {
				return fmt.Errorf("consumer %s unable to process message: %w", c.id, err)
			}
		}
	}
}
